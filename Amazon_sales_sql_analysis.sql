...
select ProductName,
       sum(Quantity) as TotalQuantity_Sold 
from amazon_sales 
group by ProductName 
order by TotalQuantity_Sold desc limit 5;
...
select Category,
       avg(TotalAmount) as Average_OrderValue 
from amazon_sales 
group by Category;
...
from 
	(select State,
             ...
             order by sum(TotalAmount)desc) as rn 
             from amazon_sales 
             group by State,City)as ranked 
where rn=1 
order by State;
...
from amazon_sales
 group by Year(OrderDate),month(OrderDate) 
order by MonthlyRevenue desc limit 1;	
...
from 
      (select City,
			  CustomerName,
              CustomerID,
              sum(TotalAmount) as TotalSpendings,
              row_number()
              over(partition by City order by sum(TotalAmount) desc)as rn 
			  from amazon_sales 
              group by City,CustomerName,CustomerID)as ranked 
where rn <=3 
order by City,rn;
...
from 
     (select Category,
             ...
             sum(TotalAmount) as TotalRevenue,
             row_number() 
             over(partition by Category order by sum(TotalAmount)desc)as rn 
             from amazon_sales 
             group by Category,ProductName)as ranked 
where rn<=3 
